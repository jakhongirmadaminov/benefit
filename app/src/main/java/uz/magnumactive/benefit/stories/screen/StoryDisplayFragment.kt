...
import uz.magnumactive.benefit.stories.customview.StoriesProgressView
import uz.magnumactive.benefit.stories.data.Story
import uz.magnumactive.benefit.stories.data.StoryUser
import uz.magnumactive.benefit.stories.utils.OnSwipeTouchListener
import uz.magnumactive.benefit.stories.utils.hide
import uz.magnumactive.benefit.stories.utils.show
...
class StoryDisplayFragment : Fragment(),
    ...
    private val stories: ArrayList<Story> by lazy { storyUser.stories }
    ...
    private fun updateStory() {
        ...
        if (stories[counter].isVideo()) {
            ...
            Glide.with(this).load(stories[counter].image!!).into(storyDisplayImage)
        }

        val cal: Calendar = Calendar.getInstance(Locale.ENGLISH).apply {
            timeInMillis = 3333333/*stories[counter].startTime!!*/
        }
    ...
    private fun initializePlayer() {
        ...
        val mediaSource = ProgressiveMediaSource.Factory(mediaDataSourceFactory).createMediaSource(
            Uri.parse(stories[counter].image)
        )
